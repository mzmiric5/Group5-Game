    ...
    public class Enemy : NPC
    {
        ...
        override public void calculate_movement(Game1 game)
        {
            ...
            if (this.aggresive == true)
            {
                ...
                if ((game.get_current_level().player.returnY() > this.returnY()) && (this.returnY() + this.returnH() + this.get_sight() >= game.get_current_level().player.returnY()))
                {
                    this.move(Direction.Up, this.movement_magnitude);
                    has_moved = true;
                }
                // if player is below enemy && player is in sight of enemy
                else if ((game.get_current_level().player.returnY() < this.returnY()) && (this.returnY() - this.get_sight() <= game.get_current_level().player.returnY() + game.get_current_level().player.returnH()))
                {
                    this.move(Direction.Down, this.movement_magnitude);
                    has_moved = true;
                }
                // if player is right of enemy && player is in sight of enemy
                else if ((game.get_current_level().player.returnX() > this.returnX()) && (this.returnX() + this.returnW() + this.get_sight() >= game.get_current_level().player.returnX()))
                {
                    this.move(Direction.Right, this.movement_magnitude);
                    has_moved = true;
                }
                // player is left of enemy && player is in sight of enemy
                else if ((game.get_current_level().player.returnX() < this.returnX()) && (this.returnX() - this.get_sight() <= game.get_current_level().player.returnX() + game.get_current_level().player.returnW()))
                {
